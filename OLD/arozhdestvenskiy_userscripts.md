...
1. Пользователь открывает раздел "Базы данных".
2. Выбирает метод ввода: вручную или загрузка файла.
3. При ручном вводе заполняет поля: компоненты, температура, давление, мольные доли, фазы.
4. При выборе загрузки — прикрепляет файл CSV/Excel с экспериментальными данными.
5. Система парсит и отображает предварительный просмотр таблицы.
6. Пользователь проверяет корректность и подтверждает импорт.
7. При наличии ошибок система сообщает об этом.
8. После валидации данные сохраняются как новый проект.
9. Пользователь переходит к выбору алгритма (см. Сценарий 2).
